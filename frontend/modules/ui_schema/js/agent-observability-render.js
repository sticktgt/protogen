...
export function renderRunObservability(run, metricsValue, eventsValue, running) {
  ...
  const idleWarning = running && idleSeconds >= 120
  ...
  return `
    <div class="agent-observability">
      <div class="agent-resource-grid">
        ${resourceCard('Время', `${formatDuration(secondsSince(run.started_at))} / ${formatDuration(limits.max_duration_seconds)}`)}
      ...
      ${idleWarning}
      <details class="agent-event-log" open>
        <summary>Журнал выполнения · ${events.length}</summary>
        <div class="agent-event-list" data-agent-event-list>
          ${events.length ? events.slice(-100).map(renderEvent).join('') : '<div class="muted-text">События ещё не получены.</div>'}
        </div>
      </details>
    </div>
  `;
