    ...
    <section class="card">
      <details class="agent-file-diff-root">
        ...
        <div class="agent-file-diff-body">
          <p class="muted-text">Точное построчное сравнение базовой и результирующей схемы. Строки с <strong>+</strong> будут добавлены, строки с <strong>−</strong> — удалены.</p>
...
function renderGroup(label, changeType, files) {
  ...
  return `
    <details class="agent-file-diff-group">
      ...
      <div class="agent-file-diff-list">
        ${group.map(renderFile).join('')}
      </div>
    </details>
  `;
}
