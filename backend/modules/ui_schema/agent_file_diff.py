...
_IGNORED_PATHS = {"index.json"}


def build_file_diff(base_root: Path, working_root: Path) -> dict[str, Any]:
    """Build an exact line-oriented diff without changing either schema."""
    ...
    *,
    ...
    diff_lines = list(
        difflib.unified_diff(
            before_lines,
            after_lines,
            fromfile="/dev/null" if before is None else f"a/{relative_path}",
            tofile="/dev/null" if after is None else f"b/{relative_path}",
            lineterm="",
        )
    )
    additions = sum(
...
def _split_lines(value: str | None) -> list[str]:
    return [] if value is None else value.splitlines()
