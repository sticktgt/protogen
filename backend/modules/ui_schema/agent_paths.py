...
def runtime_root(module_root: Path) -> Path:
    return module_root.parent / ".protoarchitect" / "ui_schema_agent"
...
def completed_runs_root(module_root: Path) -> Path:
    return runtime_root(module_root) / "completed_runs"


def completed_run_file(module_root: Path, run_id: str) -> Path:
    validate_run_id(run_id)
    return completed_runs_root(module_root) / f"{run_id}.json"


def diagnostics_root(module_root: Path) -> Path:
    return runtime_root(module_root) / "diagnostics"


def initial_snapshot_path(module_root: Path) -> Path:
    return runtime_root(module_root) / "initial" / "ui_schema.zip"
