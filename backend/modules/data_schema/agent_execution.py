...
def validate_with_retry(
    *,
    ...
    append_event(
        ...
        message="Запущена структурная проверка временной схемы данных",
    )
    ...
    for retry_index in range(retries):
        ...
        configurable["thread_id"] = validation_correction_thread_id(
            correction_config,
            run_id=run_id,
            retry_index=retry_index,
        )
        correction_config["configurable"] = configurable
        correction_config["run_name"] = f"data_schema_validation_fix_{run_id}_{retry_index + 1}"
        correction_message = (
        ...
        if not (root / "result" / "agent_completion.json").is_file():
            append_event(
                ...
                run_id,
                event_type=(
                    "validation_correction_incomplete"
                    if not validation.get("valid")
                    else "validation_finish_missing"
                ),
                level="warning",
                message=(
                    f"После попытки исправления осталось ошибок: {len(validation.get('errors', []))}"
                    if not validation.get("valid")
                    else "Backend подтвердил корректность результата самостоятельной проверкой"
                ),
            )
    if validation["valid"]:
        append_event(
...
def validation_correction_thread_id(
    invocation_config: dict[str, Any],
    *,
    run_id: str,
    retry_index: int,
    ...
    parent_thread = (
        str(configurable.get("thread_id") or "").strip()
        if isinstance(configurable, dict)
        else ""
    )
    base = parent_thread or run_id
    return f"{base}_validation_{retry_index + 1}"


def validation_retries(run: dict[str, Any]) -> int:
    config = run.get("config") if isinstance(run, dict) else None
    if not isinstance(config, dict) or "validation_retries" not in config:
        raise ValueError("agent.validation_retries must be configured")
    value = int(config["validation_retries"])
    if value < 0:
        raise ValueError("agent.validation_retries must be non-negative")
    return value
...
def record_limit_stop(module_root: Path, run_id: str, exc: Exception) -> None:
    try:
        if not is_cancelled(module_root, run_id):
            ...
            review = run.get("semantic_review", {})
            review_approved = (
                isinstance(review, dict)
                and review.get("status") in {"approved", "skipped"}
                and review.get("decision") == "approve"
                and review.get("review_kind") in {"combined", "correction_verification"}
                and review.get("coverage_complete") is True
                and not review.get("blocking")
                and not review.get("correction_pending")
                and not run.get("apply_blocked")
            )
            if validation.get("valid") and review_approved:
                append_event(
                    module_root,
                    run_id,
                    ...
                    level="warning",
                    message=(
                        "Лимит достигнут после завершённой технической и смысловой проверки; "
                        "preview формируется без нового вызова LLM"
                    ),
                )
                finalize_preview(
                    module_root,
                    ...
                    event_message=(
                        "Проверки завершены на границе лимита; формируется preview"
                    ),
                )
                return
...
def record_failure(module_root: Path, run_id: str, exc: Exception) -> None:
    try:
        if not is_cancelled(module_root, run_id):
            ...
            _LOGGER.error(
                "[data_schema_agent][%s] Unhandled agent run error",
                run_id,
        ...
        _LOGGER.exception(
            "[data_schema_agent][%s] Failed to persist agent run error", run_id
        )
...
def _build_diagnostics_safely(module_root: Path, run_id: str) -> None:
    try:
        ...
        _LOGGER.exception(
            "[data_schema_agent][%s] Failed to create diagnostics archive", run_id
        )
