...
from backend.modules.data_schema.agent_paths import history_root, initial_snapshot_path, now_iso
...
def create_snapshot(module_root: Path, *, run_id: str | None, reason: str) -> dict[str, Any]:
    ...
    create_zip(module_root, target / "data_schema.zip")
    metadata = {
...
def latest_snapshot(module_root: Path) -> dict[str, Any] | None:
    ...
    for item in root.iterdir():
        if item.is_dir() and (item / "metadata.json").is_file() and (item / "data_schema.zip").is_file():
...
def restore_latest_snapshot(module_root: Path) -> dict[str, Any]:
    from backend.modules.data_schema.agent_runs import PROCESS_LOCK, ensure_schema_writable

    with PROCESS_LOCK:
        ensure_schema_writable(module_root)
        snapshot = latest_snapshot(module_root)
        if not snapshot:
            raise FileNotFoundError("No data schema snapshots are available")
        create_snapshot(module_root, run_id=None, reason="before_snapshot_restore")
        incoming = module_root.parent / f".data_schema_restore_{uuid4().hex[:8]}"
        previous = module_root.parent / f".data_schema_restore_previous_{uuid4().hex[:8]}"
        shutil.rmtree(incoming, ignore_errors=True)
        extract_zip(Path(snapshot["path"]) / "data_schema.zip", incoming)
        validation = validate_data_schema(incoming, rebuild=True)
        ...
        try:
            ...
            if not validate_data_schema(module_root, rebuild=True)["valid"]:
                raise RuntimeError("Restored data schema is invalid")
        except Exception:
