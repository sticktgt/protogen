...
from backend.modules.data_schema.agent_paths import completed_run_file, run_root
...
def append_event(
    ...
    *,
    ...
    getattr(_LOGGER, level if level in {"debug", "info", "warning", "error"} else "info")(
        "[data_schema_agent][%s] %s", run_id, message
    )
    return event
...
def reserve_metric_slot(
    ...
    **updates: Any,
    ...
    """Atomically reserve one bounded metric slot.

    Returns the updated metrics or ``None`` when the configured maximum has
    already been reached. This is required when independent LLM batches run in
    parallel.
    """
