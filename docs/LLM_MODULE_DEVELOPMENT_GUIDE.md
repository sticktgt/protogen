...
## 21. Использование документа в публичном LLM-чате

Этот документ можно передавать в публичный LLM-чат как техническую инструкцию перед постановкой задачи на новый модуль.

Рекомендуемый порядок работы:

1. Передать модели этот документ целиком.
2. Попросить модель сначала прочитать инструкцию и кратко подтвердить понимание правил, не создавая код.
3. Отдельным сообщением передать задачу на конкретный модуль.
4. В задаче указать `module_id`, назначение модуля, страницы, API, файлы workspace и минимальные сценарии проверки.
5. Попросить модель вернуть результат по файлам, с относительными путями от корня проекта.
6. Попросить модель отдельно перечислить новые файлы, измененные файлы, инструкции подключения и команды проверки.
7. Если ответ получается большим, генерировать модуль частями: сначала конфигурационные файлы и backend, затем frontend, затем документацию.
8. После переноса файлов в проект выполнить проверки backend и frontend.
9. При ошибках компиляции или запуска передавать модели точный текст ошибки и просить исправить только затронутые файлы.

Публичный LLM-чат не имеет доступа к локальному проекту. Поэтому в запросе нужно явно указать, что модель должна выводить полный текст каждого файла, а не ссылаться на несуществующие локальные файлы.

---

## 22. Требуемый формат ответа LLM при генерации модуля

Для удобного переноса результата в проект проси модель использовать такой формат:
...
После всех файлов модель должна вывести:
...
Не нужно просить модель присылать архив. В публичном чате надежнее получать текст файлов и переносить их в проект вручную или через отдельный скрипт.

---

## 23. Требования к задаче на новый модуль

В задаче на модуль желательно указать:

- `module_id`;
- русское и техническое название модуля;
- отображается ли модуль в левом меню;
- какие страницы нужны;
- какие endpoints нужны;
- какие файлы создаются в workspace;
- какие init-файлы нужны для нового workspace;
- какие данные читает и пишет модуль;
- использует ли модуль API, страницы или файлы других модулей;
- какие проверки нужно выполнить после создания модуля.

Для минимального модуля достаточно одной страницы, одного workspace-файла и нескольких API-методов. Такой модуль удобен для проверки, что LLM поняла структуру ProtoArchitect.
